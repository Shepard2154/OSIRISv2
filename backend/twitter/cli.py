...
def _requests_response_repr(name, response, withHistory=True):
    ...
    if withHistory and response.history:
        ...
        for previousResponse in response.history:
            ret.append(f'\n    ')
            ret.append(_requests_response_repr('_', previousResponse,
                                               withHistory=False).replace('\n', '\n    '))
        ret.append('\n  ]')
...
def _requests_exception_repr(name, exc):
    ...
    ret.append(f'\n  ' + _repr(f'{name}.request',
                               exc.request).replace('\n', '\n  '))
    ret.append(f'\n  ' + _repr(f'{name}.response',
...
def _repr(name, value):
    ...
    if dataclasses.is_dataclass(value) and not isinstance(value, type):
        return f'{name} = <{type(value).__module__}.{type(value).__name__}>\n  ' + \
               '\n  '.join(_repr(f'{name}.{f.name}', f.name) + ' = ' + _repr(f'{name}.{f.name}', getattr(
...
def _dump_stack_and_locals(trace, exc=None):
    with tempfile.NamedTemporaryFile('w', prefix='snscrape_locals_', delete=False) as fp:
        if exc is not None:
            fp.write('Exception:\n')
            fp.write(
                f'  {type(exc).__module__}.{type(exc).__name__}: {exc!s}\n')
        ...
        for frameRecord in trace:
            fp.write(
                f'  File "{frameRecord.filename}", line {frameRecord.lineno}, in {frameRecord.function}\n')
...
def parse_datetime_arg(arg):
    ...
    raise argparse.ArgumentTypeError(
        f'Cannot parse {arg!r} into a datetime object')
...
def configure_logging(verbosity, dumpLocals_):
    ...
    formatter = logging.Formatter(
        '{asctime}.{msecs:03.0f}  {levelname}  {name}  {message}', datefmt='%Y-%m-%d %H:%M:%S', style='{')
...
def download_mytweets(hashtag_item):

...
class Tweet(base.Item):
	...
	username = base._DeprecatedProperty('username', lambda self: self.user.username, 'user.username')
	outlinksss = base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks) if self.outlinks else '', 'outlinks')
	tcooutlinksss = base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks) if self.tcooutlinks else '', 'tcooutlinks')
	...
	def get_data(self):
		return {
			...
			'renderedContent': self.renderedContent,
			'id': self.id,
			...
			'tcooutlinks': self.tcooutlinks,
			'media': [str(current_media) for current_media in self.media] if self.media is not None else None,
			'retweetedTweet': self.retweetedTweet,
			'quotedTweet': self.quotedTweet,
			'inReplyToTweetId': self.inReplyToTweetId,
			'inReplyToUser': self.inReplyToUser.get_data() if self.inReplyToUser is not None else None,
			'mentionedUsers': [current_user.get_data() for current_user in self.mentionedUsers] if self.mentionedUsers is not None else None,
			'coordinates': self.coordinates if self.coordinates is not None else None,
			'place': self.place if self.place is not None else None,
			'hashtags': self.hashtags if self.hashtags is not None else None,
			'cashtags': self.cashtags if self.cashtags is not None else None,
			'username': self.username,
			'outlinksss': self.outlinksss,
			'tcooutlinksss': self.tcooutlinksss,
...
class User(base.Entity):
	...
	def get_data(self):
		return {
			...
			'rawDescription': self.rawDescription,
			'descriptionUrls': self.descriptionUrls,
...
class _TwitterAPIScraper(base.Scraper):
		...
		if self._guestTokenManager.token is None:
			...
			if not self._guestTokenManager.token:
				...
				if not o.get('guest_token'):
					raise base.ScraperError('Unable to retrieve guest token')
	...
	def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
		if 'tweet' in entry['item']['content']:
			...
			raise base.ScraperException(f'Unable to handle entry {entryId!r}')
...
class TwitterSearchScraper(_TwitterAPIScraper):
	...
	def cli_setup_parser(cls, subparser):
		...
		subparser.add_argument('query', type = base.nonempty_string('query'), help = 'A Twitter search string')
...
class TwitterHashtagScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('hashtag', type = base.nonempty_string('hashtag'), help = 'A Twitter hashtag (without #)')
...
class TwitterListPostsScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('list', type = base.nonempty_string('list'), help = 'A Twitter list ID or a string of the form "username/listname" (replace spaces with dashes)')
